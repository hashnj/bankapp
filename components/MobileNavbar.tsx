...
const MobileNavbar = ({ user }:MobileNavProps) => {
  ...
  return (
    <section className='w-full max-w-[256px]'>
      <Sheet>
  <SheetTrigger>
    <Image src='/icons/hamburger.svg' width={30} height={30} className='cursor-pointer'  alt='' />
  </SheetTrigger>
  <SheetContent side={'left'} className='border-none bg-white'>
  <Link href="/" className='flex cursor-pointer item-center gap-1 px-4'>
          <Image src='/icons/logo.svg' width={34} height={34}  alt='logo' />
          <h1 className='text-26 font-ibm-plex-serif font-bold text-black-1'>N-Bank</h1>
        </Link>
